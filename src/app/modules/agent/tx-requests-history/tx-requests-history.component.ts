...
import { UploadDekontComponent } from '../../client/transactions/tx-history/upload-dekont/upload-dekont.component';

@Component({
    selector: 'tx-requests-history',
    templateUrl: 'tx-requests-history.component.html',
    styleUrls: ['tx-requests-history.component.scss'],
})
export class TxRequestsHistoryComponent {
    ...
    ngOnInit(): void {
        ...
        const from = now.getFullYear() + '-' + (+now.getMonth()) + '-' + now.getDate()
        ...
        */
        this.breadCrumbItems = [
            { label: 'TRANSACTIONS' },
            { label: 'TXREQSHISTORY', active: true }
        ];
        ...
        //     }
        // });
        this.dataService.refreshTransactionRequestsHistoryByDate(from, to)
        this.dataService.transactionsRequests$.subscribe(tx => {
            this.contentService.initContent(tx)
        })
    }
    ...
    getTxReqHistory() {
        this.dataService.refreshTransactionRequestsHistoryByDate(this.from, this.to)
    }
