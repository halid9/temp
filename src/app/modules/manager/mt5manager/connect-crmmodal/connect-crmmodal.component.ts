...
export class MT5ConnectCRMModalComponent {
  ...
  checkCRM() {
    this.loading = true;
    this.accountsService.getLiveAccountBylogin(Platform.MT5, this.account.user).subscribe({
      next: data => {
  ...
  connectToExistingClientAccount(userId: number, external: boolean = false) {
    Dialogs.confirm('Are you sure you want to connect this account to the selected client?',this.translate, () => {
      ...
      this.accountsService.connectAccount({ login: this.account.user, platform: Platform.MT5, user_Id: userId, action: 'connect', accountMasterPassword: 'Aa@123qaz', accountInvestorPassword: 'Aa@123qaz', external }).subscribe({
        next: data => {
