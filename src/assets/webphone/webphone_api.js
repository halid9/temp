...
window.webphone_api_init = initialize;
function initialize() {
    webphone_api.bd_logged = false; webphone_api.getbasedir2 = function (forceauto) {
        try {
            ...
            var scriptElements = document.getElementsByTagName('script'); if (scriptElements) { for (var i = 0; i < scriptElements.length; i++) { wpbdir = scriptElements[i].src; if (wpbdir && wpbdir.indexOf('webphone_api.js') > -1) { wpbdir = wpbdir.substring(0, wpbdir.indexOf('webphone_api.js')); if (webphone_api.bd_logged !== true) { try { if (webphone_api.parameters['logtoconsole'] != 'false' && webphone_api.parameters['logtoconsole'] != false) { console.log('EVENT, base directory - webphonebasedir(guessed): ' + wpbdir); } } catch (err) { } } webphone_api.bd_logged = true; return wpbdir; } } }
        } catch (err) { try { webphone_api.Log2('ERROR, webphone_api getbasedir2'); } catch (err2) { } } return '';
    };
